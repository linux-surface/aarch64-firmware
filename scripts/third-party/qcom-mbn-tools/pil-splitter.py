...
# Based on https://github.com/remittor/qcom-mbn-tools/blob/master/pil-splitter.py
# Changes:
# - Format using 'autopep8'
# - Convert to run with python 3 using '2to3 -w' and manual fixups
...
def gen_struct(format, image):
    ...
    return dict(list(zip(keys, elems)))
...
def parse_metadata(image):
    ...
    metadata['segments'] = []
    for i in range(metadata['num_segments']):
...
def dump_metadata(metadata, image, name):
    ...
    append_data(image, name, metadata['pg_start'],
                32 * metadata['num_segments'])
